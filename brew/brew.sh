...
while read -r tap; do    
    if ! brew tap | grep "caskroom/cask"  &>/dev/null ; then
        printf "\033[0;31m[MISSING]\033[0m $tap..."
        brew tap $tap
    else 
        printf "\033[0;32m[OK]\033[0m $tap already tapped"
    fi;
done < "./tap.txt"
...
while read -r cask; do    
    if ! brew cask ls --versions ${cask} >/dev/null 2>&1; then
        printf "\033[0;31m[MISSING]\033[0m $cask..."
        brew cask install $cask
    else 
        printf "\033[0;32m[OK]\033[0m $cask already installed"
    fi;
done < "./cask-list.txt"
...
while read -r package; do
    if ! brew ls --versions $package >/dev/null 2>&1; then
        printf "\033[0;31m[MISSING]\033[0m $package..."
...
if [[ -x "$(brew outdated)" ]]; then
    printf "[INFO] there are some outdated packages"
    brew outdated
